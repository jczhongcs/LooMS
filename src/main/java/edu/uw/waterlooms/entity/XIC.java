...
public class XIC implements Comparable<XIC>, Serializable {
  ...
  public XIC(ArrayList<Double> ints, ArrayList<Double> mzs, ArrayList<Double> rts, double peakSum, double peakArea) {
    this.intensities = ints;
    this.massChargeRatios = mzs;
    this.retentionTimes = rts;
  ...
  public void setRowInMapAtMaxIntensity(int rowInMapAtMaxIntensity) { this.rowInMapAtMaxIntensity = rowInMapAtMaxIntensity; }
  ...
  public double getCurIntensity(double dms2rt) {
    for (int i =0;i< retentionTimes.size();i++)
    {
      if(retentionTimes.get(i)==dms2rt)
